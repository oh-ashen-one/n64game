...
{
    PlayerRenderAssetsLifetime *const lifetime =
        (PlayerRenderAssetsLifetime *)context;
    assertf(
        lifetime != NULL && lifetime->recorded_block_references > 0U,
        "Player render-asset block lifetime underflow"
    );
    --lifetime->recorded_block_references;
    if (lifetime->release_requested &&
        lifetime->recorded_block_references == 0U) {
        release_owned_sprite(lifetime);
    }
}

static void retain_current_recording(PlayerRenderAssets *assets)
{
    ...
    assertf(
        ...
        "Player render-asset block lifetime overflow"
    );
...
)
{
    ...
    bool seen[BODY_PALETTE_COLORS] = {false};
    int maximum_index = -1;
    for (uint16_t y = 0U; y < pixels.height; ++y) {
        ...
        for (uint16_t x = 0U; x < pixels.width; ++x) {
            ...
            if (palette_index >= BODY_PALETTE_COLORS) {
                return false;
            }
...
bool player_render_assets_load(PlayerRenderAssets *assets)
{
    ...
    PlayerRenderAssetsLifetime *const lifetime = malloc(sizeof(*lifetime));
    if (lifetime == NULL) {
        sprite_free(body);
        return false;
    }
    *lifetime = (PlayerRenderAssetsLifetime){
        .body_sprite = body,
    };
...
{
    PlayerRenderAssets *const assets = (PlayerRenderAssets *)user_data;
    if (assets == NULL ||
        !assets->ready ||
        assets->lifetime == NULL ||
        material == NULL ||
    ...
    if (texture->texReference == PLAYER_BODY_TOP_REFERENCE) {
        region = &assets->body_regions[0];
    } else if (texture->texReference == PLAYER_BODY_BOTTOM_REFERENCE) {
    ...
    assets->successful_body_callbacks |=
        texture->texReference == PLAYER_BODY_TOP_REFERENCE ? UINT8_C(1) : UINT8_C(2);
}
