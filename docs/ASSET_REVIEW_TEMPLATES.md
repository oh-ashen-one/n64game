...
These forms turn the seven art gates into evidence-backed decisions. Copy the relevant sections into exact `review/<asset_id>/g<gate>/REVIEW.md`, replace every field, link every evidence artifact through the gate's immutable evidence manifest, and keep the history when an asset returns to an earlier gate. A review that records that manifest's path/hash stays outside the manifest and is independently hashed by its ledger, gate, or history record. A checked box without a manifest member/hash, measurement, build identifier, and reviewer observation is not evidence.
...
Before Gate 1 passes, an in-progress ordinary media concept is legal only with the ledger's exact `concept` locator grammar: canonical creator/rights holder, hashed `g1/PROVENANCE.md`, hashed `SOURCE_MANIFEST.sha256`, `output:NONE`, hashed `g1/EVIDENCE_MANIFEST.sha256` plus hashed `g1/REVIEW.md`, and seven literal `PENDING` decisions. The canonical review is the separately hashed `review:` locator outside the concept-evidence manifest; it stores the manifest's exact path/hash and must not be owned by that manifest directly or transitively. It owns only source members and Gate-1 provenance/review/evidence; `AUTHORIZATION.md`, `GATE_RECORD.tsv`, `SUBSET_EXPORT_ALLOWLIST.tsv`, `AUTHORING_STACK_RECEIPT.txt`, Gate 2–7 files, converted/output bytes, and `build/generated/` are forbidden. Acceptance also requires exact clean current `HEAD` advertised at a valid public branch or pull-request head/merge ref, plus an explicit same-ref credential-free fresh fetch whose bytes and OID match; ignored extras and symlinks in `assets-src/` or `review/` fail. A `planned` row owns no file, and the 15 fixed C7 tuples do not count as ordinary concepts.

When returning Gate 1, do not overwrite or promote the failed attempt. Add/update the single canonical `g1/attempts/ATTEMPT_HISTORY.tsv` member described in Art Bible 16.4; each row must resolve and separately hash its source/evidence/failed review at a strict public ancestor commit, while the failed review stays outside the evidence manifest whose path/hash it records. Chain the prior row bytes and record an independent reviewer, defects, substantive rationale, and exact `REJECT` or `REVISE`. The current concept review remains `IN_PROGRESS`, output remains `NONE`, and G1–G7 remain `PENDING` until a later genuine pass.
...
For the still-locked Quarrune implementation candidate, the future Gate 5 conversion is the two-phase flow in `docs/TOOLCHAIN.md`: direct `scripts/export-gate5-asset ... --candidate` from exact owner-rooted, G1–G4-authorized, source-only allowlists, then verification-only regeneration through `scripts/record-authoring-stack-receipt g5-export-pair ... --replace` after the real reviewed pair is LFS-stored, bound by exact per-owner output rows, and the implementation/hash locks are deliberately enabled in that same art transaction. Today `false/PENDING` blocks the receipt phase. Once enabled, one repository-keyed lock covers the complete source/output closure snapshot, exact Blender/Fast64 and attested Tiny3D/libdragon host tools, double export, byte comparison, postcheck, and descriptor-relative paired receipt promotion. Build each G5 evidence manifest after that sequence so its receipt is a direct `authoring.stack_receipt` member. The legacy single-scope operation, procedural fallback, arbitrary converter binary, owner swap, symlink traversal, interrupted journal, post-hoc receipt, precheck failure, exporter failure, postcheck failure, identity drift, one-sided output, or closure mutation fails. Other asset owners remain unsupported until a separately reviewed exporter extension exists.
...
- [ ] Confirm the canonical clean-staging exporter and implementation module are exact checker-bundle members. While the Phase-1 boundary remains `GATE5_EXPORT_IMPLEMENTED=false` with a `PENDING` pin, Gate 5 is locked. A later audited change may set the flag true only when the executable hash, canonical authored inputs, closure seals, reproducible tool provenance, pair validation, visual decisions, and native evidence all support that transition.
