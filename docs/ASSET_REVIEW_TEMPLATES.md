...
For the still-locked Quarrune implementation candidate, the future Gate 5 conversion is the two-phase flow in `docs/TOOLCHAIN.md`: direct `scripts/export-gate5-asset ... --candidate` from lawfully authorized source-only allowlists, then `scripts/record-authoring-stack-receipt g5-export-pair ... --replace` only after the real reviewed pair is LFS-stored, bound by exact output rows, and the implementation/hash locks are deliberately enabled in that same art transaction. Today `false/PENDING` blocks the receipt phase. Once enabled, the wrapper verifies the exact Blender/Fast64 stack, invokes the pinned paired exporter without a shell, regenerates and byte-compares both review snapshots and output manifests, verifies the stack again, requires the portable identities to match, then rollback-protects both owner receipts. Build each G5 evidence manifest after that sequence so its receipt is a direct `authoring.stack_receipt` member. The legacy single-scope operation, a post-hoc receipt, precheck failure, exporter failure, postcheck failure, identity drift, one-sided output, or output mutation fails. Other asset owners remain unsupported until a separately reviewed exporter extension exists.
