...
| 4. Visual benchmark | In progress | Approved Annex composition, player, four-Echoform battle-distance set, lighting, UI, animation, and VFX in-engine | Pinned Blender 4.5.11/Fast64 2.5.3 authoring checks pass, but the project still has zero authored production model/texture packages. The deterministic Gate-5 exporter, reviewed Tiny3D outputs, native-camera captures, and visual approval remain missing |
| 5. Playable spine and gameplay | In progress | Cold boot through stable beacon ending; one battle; Annex exploration; save/retry; timing instrumentation and edge cases | Portable cold-boot-to-beacon state machine, exact 106-tick slate, name entry, Annex objective/dialogue, controller-driven 2v2 command/target/queue flow, victory/defeat/retry, controller-loss freeze, canonical checkpoints, and dual-slot footer-last EEPROM journal are implemented. Strict MIPS build and 114 host tests pass; current ROM is 229,376 bytes with SHA-256 `9785ae6e...ea239c`. Real-controller full-route and timed certification remain open; current geometry is explicitly temporary |
