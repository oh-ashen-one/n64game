...
- Source paths use `assets-src/<production-id-prefix>/<canonical.production.id>/`, where the first directory is byte-for-byte the ID segment before its first dot (for example `assets-src/chr/chr.player.ari/`); generated runtime outputs use `build/generated/assets/<production-id-prefix>/`. Evidence and records use `review/<canonical.production.id>/g<gate>/`.
...
`review/benchmark/WHITELIST_GATE_REGISTRY.tsv` has this exact 23-field header and exactly 46 following rows in `WB-001..WB-046`/control-whitelist order:
...
The run record binds clean-build ID, exact ROM/capture/audit/execution-log bytes, `ares_version=148`, revision `0aafd85789215e84e1e43415c07d4c88461b7899`, audited Ares executable digest, `Homebrew_Mode`, three `RGBA16` framebuffers, payload-owned `scripts/run-ares@<sha256>`, wrapper version `n64game-ares-wrapper-v1`, exact invocation, canonical independent operator, and strict RFC-3339 time. Approval reruns that reviewed wrapper in a fresh public clone with the exact local ROM, Ares binary, MP4, audit, event log, and run record. It must emit one exact JSON verification receipt binding all six digests; a bare operator attestation cannot pass. Gate 3 must replace both fail-closed `PENDING` pins for the audited Ares binary and full approval-lifecycle fixture suite before any `APPROVED` state is reachable. The pinned fixture manifest is exact `test/fixtures/asset_contract/APPROVED_LIFECYCLE_MANIFEST.sha256`, a direct benchmark-payload member; it owns executable `scripts/test-asset-contract-lifecycle` plus populated, approved, repair, generated-child, move-pair, H2, and release cases. Approval executes that reviewed runner and requires its exact manifest/case/PASS JSON receipt, so typing a 64-hex pin without the audited bytes cannot unlock production.
...
Rows are exact `1..N` / `G1-0001..`, each attempt commit is a strict credential-free public ancestor of the current concept commit and a strict forward descendant of the preceding attempt, timestamps strictly increase, and `previous_row_sha256` is `NONE` then SHA-256 of the preceding complete TSV data row plus LF. Each row resolves its historical source manifest plus canonical `review/<id>/g1/attempts/%04d/EVIDENCE_MANIFEST.sha256` and `REVIEW.md` at that exact commit. The historical manifest owns the review as exact `gate.attempt_review`, capture/build `-`. The review has ordered keys `schema, production_id, attempt_id, gate, decision, reviewer_id, reviewer_non_owner, source_manifest_sha256, output_manifest_sha256, evidence_manifest, build_id, decided_at, defect_ids, disposition, rationale`, schema `n64game-g1-attempt-review-v1`, `gate: G1`, `decision: fail`, `output_manifest_sha256: NONE`, `build_id: -`, independent canonical reviewer, sorted defects, substantive rationale, and exact `REJECT` or `REVISE`. The live ledger and concept review remain `concept`, `IN_PROGRESS`, output `NONE`, and all seven gates `PENDING`; attempt history neither advances G1 nor orphans the rejected evidence.
...
- Signed Gate 1–7 templates for the hero assets and environment sector.
