...
Gate-4 benchmark authority is split by canonical production ID, not by character nickname. An `echo.*` row is authorized only for its listed model, texture, rig, and shadow selectors and is authorized for no animation source or output; its paired `anm.echo.*` row exclusively owns the authorized clip selectors. For Quarrune, `echo.quarrune` owns exactly five model selectors, while `anm.echo.quarrune` exclusively owns the nine `S1` benchmark clips listed above. Character-level rollups are non-owning aggregates and neither create nor merge source/output authority. The typed Tiny3D package reader and model/animation skeleton binding are necessary byte-safety prerequisites only. Gate 5 remains hard-locked until the canonical texture/blob-shadow output contract and clean-staging audited exporter are implemented, hash-pinned, quality-reviewed, and deliberately enabled with every existing visual, technical, and evidence requirement still satisfied.
