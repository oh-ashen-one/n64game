...
Gate-4 benchmark authority is split by canonical production ID, not by character nickname. An `echo.*` row is authorized only for its listed model, texture, rig, and shadow selectors and is authorized for no animation source or output; its paired `anm.echo.*` row exclusively owns the authorized clip selectors. For Quarrune, `echo.quarrune` owns exactly five model selectors, while `anm.echo.quarrune` exclusively owns the nine `S1` benchmark clips listed above. Character-level rollups are non-owning aggregates and neither create nor merge source/output authority. Gate 5 remains hard-locked until typed Tiny3D package decoding and model/animation skeleton binding land atomically with the audited exporter.
