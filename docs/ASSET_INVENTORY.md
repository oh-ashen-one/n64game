...
|---|---|---|---|---|---|
| `echo.quarrune` | **Quarrune** — real starter A; compact six-legged ceramic ram, cobalt horn lattice, tank/support | Strata: Ridge Ram; Brace Relay; Grounding Ring; Steady Pulse | 1,250 / 650 tris; 3 mats; 64×64 CI8 + 32×32 CI4; 20 joints | 14 total in paired `anm.echo.quarrune`; this model row owns no animation clips. `S1` requires `idle_a`, `idle_b`, `entrance`, `reposition`, `ridge_ram`, `brace_relay`, `hit`, `knockout`, and `horizon_break` participation preview from that bank; the remaining five bank clips are `S4`. | `S1/S4 P0` |
| `echo.ayselor` | **Ayselor** — real starter B; hovering tri-wing manta/kite, clothlike fins, amber keel lamp | Gale: Sirocco Slice; Lift Current; Dazzle Wake; Guiding Draft | 1,150 / 600 tris; 3 mats; 64×64 CI8 + 32×32 CI4; 18 joints | 15 total in paired `anm.echo.ayselor`; this model row owns no animation clips. The model's `S1` subset is only its final 600-triangle distance model, texture, rig, and blob shadow; the paired bank owns `idle_a`, `reposition`, and `hit` at `S1`, with its remaining clips at `S4`. | `S1 subset P0 / S4 remainder P1` |
...
| `echo.gyreclast` | **Gyreclast** — simulation opponent A; three-legged mineral excavator, offset drill claw | Strata: Auger Knuckle; Dust Screen; Fault Pin; Carapace Brace | 1,200 / 620 tris; 3 mats; 64×64 CI8 + 32×32 CI4; 18 joints | 11 total in paired `anm.echo.gyreclast`; this model row owns no animation clips. The model's `S1` subset is only its final 620-triangle distance model, texture, rig, and blob shadow; the paired bank owns `idle_a`, `reposition`, and `hit` at `S1`, with its remaining clips at `S4`. | `S1 subset P0 / S4 remainder P1` |
| `echo.kivarrax` | **Kivarrax** — simulation opponent B; long-legged fan-tailed desert runner | Gale: Crosswind Cut; Slipstream; Pressure Drop; Talon Sweep | 1,050 / 540 tris; 3 mats; 64×64 CI8 + 32×32 CI4; 20 joints | 11 total in paired `anm.echo.kivarrax`; this model row owns no animation clips. The model's `S1` subset is only its final 540-triangle distance model, texture, rig, and blob shadow; the paired bank owns `idle_a`, `reposition`, and `hit` at `S1`, with its remaining clips at `S4`. | `S1 subset P0 / S4 remainder P1` |
...
Gate-4 benchmark authority is split by canonical production ID, not by character nickname. An `echo.*` row is authorized only for its listed model, texture, rig, and shadow selectors and is authorized for no animation source or output; its paired `anm.echo.*` row exclusively owns the authorized clip selectors. For Quarrune, `echo.quarrune` owns exactly five model selectors, while `anm.echo.quarrune` exclusively owns the nine `S1` benchmark clips listed above. Character-level rollups are non-owning aggregates and neither create nor merge source/output authority. The typed Tiny3D/sprite package readers, runtime-helper binding, and model/animation skeleton binding are necessary byte-safety prerequisites only. Gate 5 remains hard-locked until the clean-staging audited exporter, canonical real source/output art, all visual reviews, native proof, and deliberate implementation pins exist with every visual, technical, and evidence requirement still satisfied.
