...
The candidate command requires the exact manifest-owned, materialized Git-LFS `.blend` files and three authored paletted PNGs; it has no procedural model, texture, animation, or placeholder fallback. The current repository has no visually accepted canonical Quarrune source pair, so this precondition is intentionally unsatisfied. After a later lawful art transaction supplies and passes the real source, stores the reviewed pair through canonical LFS, represents it by exact `OUTPUT:runtime` allowlist rows, and deliberately flips/pins the exporter locks, regenerate and byte-compare it while producing both receipts in one rollback-protected operation:
