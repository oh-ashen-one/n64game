...
| Derived state | Eight-bit aggregate mask | Required facts |
|---|---:|---|
| `EMPTY` | `00000000` | no ordinary concepts, no advanced rows, and all 52 authorization rows `INACTIVE` with seven `PENDING` gates |
| `PUBLIC_HEAD_CONCEPT` | `00000000` | at least one ordinary concept, no advanced rows, all 52 authorization rows inactive, and exact clean current public-ref authority verified through a fresh explicit-ref clone |
| `STAGED_WB` | `11xxxxxx` | payload plus whitelist are jointly populated and at least one whitelist row is `AUTHORIZED` or lawful `REPAIR_ONLY` |
| `APPROVED` | `11111111` | all pairs and all approval requirements pass |

The eight mask positions are exactly: (1) payload manifest, (2) whitelist/gate registry, (3) benchmark evidence registry, (4) ROM build recipe, (5) player rollup, (6) Quarrune rollup, (7) benchmark-sector rollup, and (8) integrated-presentation rollup. Each path/hash pair is either literal `PENDING`/`PENDING` or its exact positional canonical path plus recomputed digest. Payload and whitelist are inseparable; every optional bit implies core `11`. The recipe may be added independently after the core. `Public build workflow file` may remain `PENDING` after the recipe is populated, but it cannot be populated unless the recipe is populated. A present rollup binds its exact aggregate digest and the global clean-build ID. An ordinary active staged row has its own canonical build ID and need not equal the global clean-build ID unless evidence or approval is claimed.

The evidence bit claims the complete gate graph. It is legal only with all 52 whitelist rows active, all four rollup pairs and records present, and all `392 = (52 + 4) × 7` decisions legal completions; every evidence, row, and rollup build then equals the global clean-build ID. Every populated aggregate is owned by one shared commit-bound payload-manifest context, and no aggregate/member is trusted from mutable worktree bytes. No unowned regular file may hide under `review/benchmark/`. Recipe size/header/output metadata is structurally checked before approval; equality to local/fresh/public ROM bytes is approval-only because those current ROM identity fields remain `PENDING`. Prior approved controls, evidence, and decisions remain immutable under their signed historical tags rather than being copied as stale current authority.

`PUBLIC_HEAD_CONCEPT` accepts only exact clean current `HEAD` at the canonical public repository through one advertised `refs/heads/*`, `refs/pull/<positive>/head`, or `refs/pull/<positive>/merge` ref. The verifier resolves that OID, explicitly fetches the same ref in a no-checkout credential-free fresh clone, rechecks the OID, and reads all accepted concept bytes there. Local-only or dirty commits, moved refs, tags, implicit default-branch clones, and local ignored extras or symlinks under `assets-src/` or `review/` fail.

`STAGED_WB` uses a public two-commit payload/control transaction. Commit `P` contains the reviewed payload and is exactly `Reviewed payload Git commit`; control commit `A` has exactly one parent `P`, and `P→A` changes only `docs/VISUAL_BENCHMARK_APPROVAL.md`. Exact clean current public `HEAD` must descend from `A`, both commits must remain publicly reachable through the verified authority, the control bytes at `A` must equal the current control bytes, and current `HEAD^{tree}` must equal `A^{tree}` byte-for-byte. This tree equality permits a synthetic pull-request merge commit with an identical tree while rejecting every post-`A` byte change. A same-repository but unrelated public payload commit is not valid authority.

A populated rollup requires the whitelist/gate registry. Every rollup gate row binds source/output SHA fields to that exact registry digest, not to the parent payload-manifest digest; this keeps the payload graph acyclic. These branch and pull-request authorities are confined to preapproval concept/staging. The postapproval production rule still requires exact credential-free public default `HEAD` as specified in Art Bible section 16.7.
...
| `echo.quarrune` | final hero/distance model, texture, rig, blob shadow only; owns no animation clips | every animation clip; the nine benchmark clips are separately and exclusively owned by `anm.echo.quarrune` |
...
- Quarrune remains the benchmark's only hero Echoform. `echo.quarrune` exclusively owns its five model-package selectors (hero model, distance model, texture, rig, and blob shadow); `anm.echo.quarrune` exclusively owns its nine benchmark animation selectors. Character-level `rollup.quarrune` is a non-owning aggregate bound to the registry and global clean build; it neither creates nor merges source/output authority. Ayselor, Gyreclast, and Kivarrax are three fully finished supporting battle-distance silhouettes needed to make the representative 2v2 scene truthful. Each supporting `echo.*` subset retains its canonical `RIGGED_MODEL / H2` seven-gate and two-polish-pass obligations, but authorizes only its exact distance-model/texture/rig/blob-shadow selectors; its paired `anm.echo.*` row authorizes only `idle_a`, `reposition`, and `hit`. Hero meshes, all other clips, move VFX/audio, portraits, vocals, and every other full-package member remain locked.
