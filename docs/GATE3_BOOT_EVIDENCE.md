...
The machine-readable identity record is [`captures/gate3/evidence.json`](../captures/gate3/evidence.json). Host tests recompute both PNG hashes, byte sizes, PNG signatures, dimensions, distinct-frame requirement, ROM/Ares identity, and CI identifiers against it and `SHA256SUMS`.
...
**BLOCKED on this host; Gate 3 therefore remains open.** The unchanged architecture contract requires the exact pins to build locally and in clean CI. Clean CI and local Ares boot pass, but the macOS Docker engine is unavailable. Docker Desktop 4.82.0 build 233772 is installed from the exact Homebrew ARM64 archive SHA-256 `2da717ef1ca2ae0240a68458e0aaee32be9bd9fe574fd916dd43dae40f17c12c`; the app's deep Developer ID signature for Docker Inc team `9BNSXJN65R` passes and the official DMG notarization ticket validates. The host policy service nevertheless reports `failed to call driver: 0x3`, kills or stalls the backend before `dyld` startup, and never creates a Docker socket. A clean local `make rom` plus report/ROM comparison is still mandatory before this gate can be marked complete.
