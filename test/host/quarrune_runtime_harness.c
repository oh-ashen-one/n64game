...
sprite_t *sprite_load(const char *path)
{
    ...
    if (body) {
        ...
        for (int index = 0; index < sprite->palette_colors; ++index) {
            sprite->palette[index] = (uint16_t)((uint16_t)index * UINT16_C(2) + UINT16_C(1));
        }
...
int sprite_get_palette_used_colors(const sprite_t *sprite) { return sprite->palette_colors; }
...
{
    return (surface_t){
        .buffer = (uint8_t *)surface->buffer + (size_t)y * (size_t)surface->stride + x,
...
tex_format_t surface_get_format(const surface_t *surface) { return surface->format; }
bool rdpq_tex_can_upload(const surface_t *surface) { return surface_bytes(surface) <= 2048U; }
int rdpq_tex_upload(
    rdpq_tile_t tile,
    const surface_t *surface,
...
int main(void)
{
    ...
    T3DMaterial top = body_material(QUARRUNE_BODY_TOP_REFERENCE);
    T3DMaterial bottom = body_material(QUARRUNE_BODY_BOTTOM_REFERENCE);

    QuarruneRenderAssets immediate = {0};
    assert(quarrune_render_assets_load(&immediate));
    assert(!quarrune_render_assets_load(&immediate));
    assert(!quarrune_render_assets_callback_ok(&immediate));
    quarrune_render_assets_dynamic_texture_cb(&immediate, &top, &parameters, TILE0);
    assert(!quarrune_render_assets_callback_ok(&immediate));
    quarrune_render_assets_dynamic_texture_cb(&immediate, &bottom, &parameters, TILE0);
    assert(quarrune_render_assets_callback_ok(&immediate));
    assert(quarrune_render_assets_upload_blob_shadow(&immediate, TILE0, &parameters));
    assert(texture_upload_count == 3);
    assert(palette_upload_count == 2);
    assert(mipmap_none_count == 3);
    assert(quarrune_render_assets_unload(&immediate));
    assert(sprite_free_count == 2);
    assert(wait_count == 1);

    QuarruneRenderAssets deferred = {0};
    assert(quarrune_render_assets_load(&deferred));
    recording = true;
    quarrune_render_assets_dynamic_texture_cb(&deferred, &top, &parameters, TILE0);
    quarrune_render_assets_dynamic_texture_cb(&deferred, &bottom, &parameters, TILE0);
    assert(quarrune_render_assets_upload_blob_shadow(&deferred, TILE0, &parameters));
    recording = false;
    assert(exit_callback_count == 3U);
    assert(!quarrune_render_assets_unload(&deferred));
    assert(!deferred.ready && deferred.lifetime == NULL);
    assert(sprite_free_count == 2);
    free_recorded_block();
    assert(sprite_free_count == 4);

    QuarruneRenderAssets faulted = {0};
    assert(quarrune_render_assets_load(&faulted));
    quarrune_render_assets_dynamic_texture_cb(&faulted, &top, &parameters, 1);
    assert(!quarrune_render_assets_callback_ok(&faulted));
    assert(quarrune_render_assets_unload(&faulted));
    assert(sprite_free_count == 6);

    puts("quarrune runtime harness: PASS");
    return 0;
}
