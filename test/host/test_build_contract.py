...
class BuildContractTests(unittest.TestCase):
    ...
    def test_ari_candidate_uses_exact_conversion_animation_and_renderer_contract(self) -> None:
        makefile = (ROOT / "mk" / "rom.mk").read_text(encoding="utf-8")
        self.assertIn(
            "--base-scale=64 --asset-path=$(ARI_SOURCE_DIR)/filesystem --verbose",
            makefile,
        )
        self.assertIn(
        ...
        renderer = (ROOT / "src" / "n64game_render.c").read_text(encoding="utf-8")
        self.assertIn('"rom:/chr/player_ari/ari.t3dm"', renderer)
        self.assertIn('"rom:/chr/player_ari/tex_ari_body_ci8_64x64.sprite"', renderer)
        self.assertIn('{"idle_a", "walk", "run"}', renderer)
        self.assertIn("n64game_indexed_render_assets_dynamic_texture_cb", renderer)
        self.assertIn(".minimum_body_palette_colors = 24U", renderer)
        self.assertIn("t3d_skeleton_blend", renderer)
