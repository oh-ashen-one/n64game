...
This is a playable implementation milestone, not a release or visual-quality claim. The Annex geometry, skinned/animated Ari player, all four battle Echoforms, and the user-selected `Meridian Annex Drift` music are reviewed runtime candidates. Quarrune, Ayselor, Gyreclast, and Kivarrax now replace the battle stand-ins with original 3D models and event-driven `idle_a`, `reposition`, and `hit` clips. The music is preserved with its Suno metadata, transformed into a seamless 68.08-second mono loop, deterministically compressed below 300 KiB, and played with scene-aware fades through exploration and battle. A fail-closed post-build audit verifies the audio source/output identities alongside all four skeletons, clips, material-to-texture bindings, hardware palette profiles, and the exact reviewed Quarrune/Ayselor Tiny3D bytes. Quarrune and Ayselor are copied from the host-reviewed packages because a separately audited container reconversion produced different bytes; their source GLBs remain hash-locked as provenance. The repository still contains **zero approved production 3D/texture asset packages**, and the music remains a development candidate pending commercial-rights confirmation and native mix approval. Final native-camera art approval, remaining audio/SFX, VFX, lighting, camera staging, real-time pacing, and the complete timed Ares run remain unverified. The reduced scope cuts the Estate, world map, second battle, follower system, and extra four Echoforms so remaining work can concentrate on a coherent finished slice without lowering the art bar.
...
When launched through `scripts/run-ares` on the pinned macOS Ares v148 build, the wrapper forces the native Quartz input driver and supplies a deterministic keyboard map. Arrow keys and `WASD` drive both the N64 D-pad and control-stick directions, so the same keys work in menus and during movement. Keyboard `X` is N64 `A`, keyboard `Z` is N64 `B`, `Space` is N64 C-down / Field Relay, `Left Shift` is N64 `Z`, and `Return` is N64 `Start`.
